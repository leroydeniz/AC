...
void generar_pagina_encriptada(pagina in_page, pagina *out_page) {
	...
	for (int height=0;height<in_page.h;height++) {
		for (int width=0;width<in_page.w;width=width+2) {
			unsigned char pixel1 = in_page.im[height][width];
			unsigned char pixel2 = in_page.im[height][width+1];
